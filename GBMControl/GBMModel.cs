...
namespace GBMControl
{
    public class GBMModel : RModelInterface
    {
        ...
        public Dictionary<string, object> PackState(string SerializedModel = null)
        {
            Dictionary<string, object> dictPackedState = new Dictionary<string, object>();
        
            //Pack the model state into a dictionary.
            dictPackedState.Add("data", data_dict);
            dictPackedState.Add("target", target);
            dictPackedState.Add("threshold", threshold);
            dictPackedState.Add("regulatory", regulatory);
            dictPackedState.Add("specificity", specificity);
            dictPackedState.Add("iterations", iterations);
            dictPackedState.Add("cost", cost);
            dictPackedState.Add("depth", depth);
            dictPackedState.Add("shrinkage", shrinkage);
            dictPackedState.Add("weights", weights);
            dictPackedState.Add("trees", trees);
            dictPackedState.Add("folds", folds);
            dictPackedState.Add("fraction", fraction);
            dictPackedState.Add("minobsinnode", minobsinnode);

            if (SerializedModel == null) { dictPackedState.Add("modelstring", utils.SerializeRObject(this.model)); }
            else { dictPackedState.Add("modelstring", SerializedModel); }
        ...
        public void UnpackState(Dictionary<string, object> PackedState)
        {
            ...
            Type objType = typeof(Dictionary<string, double[]>);
            string jsonRep = PackedState["data"].ToString();
            this.data_dict = (Dictionary<string, double[]>)(JsonConvert.DeserializeObject(jsonRep, objType));

            //Unpack the weights
            objType = typeof(double[]);
            jsonRep = PackedState["weights"].ToString();
            this.weights = (double[])(JsonConvert.DeserializeObject(jsonRep, objType));

            //Unpack the cost array
            objType = typeof(double[]);
            jsonRep = PackedState["cost"].ToString();
            this.cost = (double[])(JsonConvert.DeserializeObject(jsonRep, objType));

            //Unpack the rest of the model object
            this.target = PackedState["target"].ToString();
            this.specificity = Convert.ToDouble(PackedState["specificity"]);
            this.trees = Convert.ToInt32(PackedState["trees"]);
            this.actual = data_dict[target];
            this.iterations = Convert.ToInt32(PackedState["iterations"]);
            this.depth = Convert.ToInt32(PackedState["depth"]);
            this.shrinkage = Convert.ToDouble(PackedState["shrinkage"]);
            this.folds = Convert.ToInt32(PackedState["folds"]);
            this.fraction = Convert.ToDouble(PackedState["fraction"]);
            this.minobsinnode = Convert.ToInt32(PackedState["minobsinnode"]);

            //Get the data into R 
            this.data_frame = utils.DictionaryToR(this.data_dict);
            this.num_predictors = this.data_dict.Keys.Count - 1;
            
            //Restore the model object in R
            this.model = utils.DeserializeRObject(PackedState["modelstring"].ToString());
            
            GetFitted();

            //Establish a decision threshold
            this.specificity = Convert.ToDouble(PackedState["specificity"]);
            this.threshold = Convert.ToDouble(PackedState["threshold"]);
            this.regulatory = Convert.ToDouble(PackedState["regulatory"]);
        }
        
        
        public void Create(Dictionary<string, double[]> data, string target, double regulatory = 2.3711, int iterations = 10000, double[] cost = null, double specificity = 0.9, int depth = 5, int minobsinnode = 5, double shrinkage = 0.001, double fraction = 0.5, int folds = 0, string weights = "none")
        {
        ...
        public Dictionary<string, double> GetInfluence()
        {
            ...
            IntegerVector variableOrder = RInterface.R.GetSymbol("i").AsInteger();
            
            List<string> vars = (from i in variableOrder select names[i-1]).ToList<string>(); //the i-1 is because R indexes from 1 while C# indexes from 0.
            ...
            Dictionary<string, double> result = vars.Zip(influence, (k, v) => new { Key = k, Value = v }).ToDictionary(x => x.Key, x => x.Value);
