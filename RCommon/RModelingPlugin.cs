...
{
    //Modeling Plugin
    public abstract class RModelingPlugin : Extension, IPartImportsSatisfiedNotification, IPlugin
    {       
        [Import("Shell")]
        ...
        //instance of  class
        protected RModelingGui modelingControl;
        ...
        public override void Activate()
        {
            ...
            modelingControl.ModelUpdated += new EventHandler(HandleUpdate);
            modelingControl.ModelingTabControl.SelectedIndexChanged += new EventHandler(UpdateControlStatus);
            modelingControl.ModelingCompleteEvent += new EventHandler(ModelingComplete);
            modelingControl.ModelingCanceledEvent += new EventHandler(ModelingCanceled);
        ...
        public void AddRibbon(string sender)
        {
            ...
            btnComputeAO = new SimpleActionItem(strPanelKey, "Compute A O", modelingControl.btnComputeAO_Click);
            ...
            btnManipulate = new SimpleActionItem(strPanelKey, "Manipulate", modelingControl.btnManipulate_Click);
            ...
            btnTransform = new SimpleActionItem(strPanelKey, "Transform", modelingControl.btnTransform_Click);
        ...
        {
            //App.DockManager.SelectPanel(strPanelKey);
            var dp = new DockablePanel(strPanelKey, strPanelCaption, modelingControl, DockStyle.Fill);
            ...
            if (((IPlugin)sender).PluginType == Globals.PluginType.Datasheet)
            {
                ...
                else
                {
                    if (!(bool)e.PackedPluginState["Clean"])
                    {
                        boolComplete = false;
                        boolVirgin = false;
                        modelingControl.Clear();
                        modelingControl.SetData(e.PackedPluginState);
                        boolChanged = true;
                    }
                    Show();
                }
        ...
        public void UpdateControlStatus(object sender, EventArgs e)
        {
            try
            {
                string strActiveTabName = modelingControl.ModelingTabControl.SelectedTab.Name.ToString();
        ...
        public void Broadcast()
        {
            ...
            IDictionary<string, object> dictPackedState = GetPackedState();
        ...
        {
            //get packed state, add complete and visible and raise broadcast event
            IDictionary<string, object> dictPackedState = modelingControl.PackState();
            ...
            dictPackedState.Add("Complete", modelingControl.Complete);
        ...
        private void ProjectSavedListener(object sender, VBCommon.PluginSupport.SerializationEventArgs e)
        {
            ...
            IDictionary<string, object> packedState = modelingControl.PackState();
        ...
        private void ProjectOpenedListener(object sender, VBCommon.PluginSupport.SerializationEventArgs e)
        {
            if (e.PackedPluginStates.ContainsKey(strPanelKey))
            {
                ...
                modelingControl.UnpackState(dictPlugin);
        ...
        private void PushToStack(object sender, UndoRedoEventArgs args)
        {
            if (boolChanged)
            {
                IDictionary<string, object> packedState = modelingControl.PackedState;
        ...
        void btnRun_Click(object sender, EventArgs e)
        {
            ...
            if (modelingControl.lbIndVariables.Items.Count == 0)
            {
            ...
            //If a random seed has been provided, make sure it is valid before applying it.
            if (modelingControl.cbRandomSeed.Checked)
            {
                if (modelingControl.tbRandomSeed.Text != "")
                {
                    int intRandomSeed;
                    if (int.TryParse(modelingControl.tbRandomSeed.Text, out intRandomSeed))
                    {
                        RInterface.R.Evaluate("set.seed(" + modelingControl.tbRandomSeed.Text + ")");
                        utils.SeedRNG(intRandomSeed);
                    }
                    else
                    {
                        MessageBox.Show("If you are going to supply a random seed, it must be an integer.");
                        return;
                    }
                }
                else
                {
                    MessageBox.Show(@"Error: the ""Set Seed Value"" box is checked, but you haven't specified a random seed.");
                    return;
                }
            }
            ...
            bool bSuccess = modelingControl.btnRun_Click(sender, e);
        ...
        {
            modelingControl.btnCancel_Click(sender, e);
            boolComplete = false;
        }
        ...
        {
            modelingControl.btnRemoveInputVariablesFromModelingTab_Click(sender, e);
            boolComplete = false;
        }
