...
namespace VBCommon.Expressions
{
    ...
    {
        protected Dictionary<string , Globals.Operations> dictOperations;
        ...
        private void Init()
        {
            dictOperations = new Dictionary<string, Globals.Operations>();
        ...
        private void PRODUCT(Expression expression)
        {
            ...
            /// <summary>
        /// Subtract the remaining values of all the fields in the expression from the first value
        /// </summary>
        /// <param name="expression"></param>
        private void DIFFERENCE(Expression expression)
        {
            string newColName = expression.ToString();
            DataColumn dc = AddColumnToDataTable(expression);
            ...
            if (dc != null)
            {
                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    double diff = Convert.ToDouble(_dt.Rows[i][expression.Variables[0]]);
                    for (int j=1; j < expression.Variables.Count; j++)
                    {
                        diff = diff - Convert.ToDouble(_dt.Rows[i][expression.Variables[j]]);
                    }
                    _dt.Rows[i][newColName] = diff;
                }
            }
        }
