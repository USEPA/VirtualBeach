...
namespace VBCommon.Controls
{
    ...
    public partial class frmPlot : Form
    {
        ...
        public frmPlot(int responsevarcol, int selectedcol, DataTable dt, out string message)
        {
            InitializeComponent();
        ...
        private void showColInfo()
        {
            ...
            lvi = new ListViewItem("A-D Statistic");
            lvi.SubItems.Add(norm.ToString("n4"));
            listView1.Items.Add(lvi);

            lvi = new ListViewItem("A-D Stat P-Value");
        ...
        private void renderPlots(out string message)
        {
            ...
            GraphPane gpSV = addPlotXY(response, iv, tags, out message);
        ...
        /// <returns></returns>
        private GraphPane addPlotXY(double[] response, double[] iv, string[] tags, out string message)
        {
            ...
            //changed 11/21/2014 by mog - added if clause for highly correlated response/iv vars (pval blowsup)
            if (_dt.Rows.Count > 6 && (Statistics.Statistics.Correlation(response, iv) < 1.0))
            {
                ...
                //generate some warnings if can't plot regression line
            else if (_dt.Rows.Count < 7)
            {
                //MessageBox.Show("Too few observations to plot regression line on scatter plot", MessageBoxIcon.Warning.ToString());
                message = "Too few observations to plot regression line on scatter plot";
            }
            else
            {
                //MessageBox.Show("Response variable " + _dt.Columns[intResponsevarcol].Caption + 
                //    " is highly correlated with " + _dt.Columns[intSelectedcol].Caption + ".\nValues are similar or identical;" +
                //     " no scatter plot regression line can be generated.  \nReview you imported data; programmatic difficulties with model building is likely.",
                //    MessageBoxIcon.Warning.ToString());
                message = "Response variable " + _dt.Columns[intResponsevarcol].Caption +
                    " is highly correlated with " + _dt.Columns[intSelectedcol].Caption + ".\nValues are similar or identical;" +
                     " no scatter plot regression line can be generated.  \nReview you imported data; programmatic difficulties with model building is likely.";
            }
        ...
        private bool zgc_MouseDoubleClick(ZedGraphControl sender, MouseEventArgs e)
        {
            return default(bool);
        }
