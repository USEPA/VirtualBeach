...
namespace VBCommon.Controls
{
    ...
    public partial class frmPcoeff : Form
    {
        ...
        private void frmPcoeff_Load(object sender, EventArgs e)
        {
            ...
            if (varname == null || varname.Count < 1)
            {
                DataTable dtMsg = new DataTable();
                dtMsg.Columns.Add("Message");
                DataRow dr = dtMsg.NewRow();
                dr[0] = "No variables available for transform.";
                dtMsg.Rows.Add(dr);
                dataGridView1.DataSource = dtMsg;
                return;
            }
            ...
            for (int i = 0; i < varname.Count; i++)
            {
                ...
                double pval;
                //addded test for highly correlated columns - mog 11/21 2014
                if (pCoeff[i] < 1.0)
                    pval = VBCommon.Statistics.Statistics.Pvalue4Correlation(pCoeff[i], _dtCopy.Rows.Count);
                else
                    pval = double.NaN;
        ...
        private void viewPlotsEH(object o, EventArgs e)
        {
            ...
            frmPCPlots frmPlots = new frmPCPlots(variable, plotvarnames, _depvarname, _dtCopy, (DataTable)dataGridView1.DataSource, out message);
            //frmPlots.ShowDialog();
            frmPlots.Show();
