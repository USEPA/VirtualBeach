...
namespace VBCommon.Controls
{
    ...
    public partial class frmTransform : Form
    {
        ...
        private void cbSelectAll_CheckedChanged(object sender, EventArgs e)
        {
            ...
            System.Windows.Forms.Control.ControlCollection cbs = gbIVs.Controls;
            if (cbSelectAll.Checked)
            {
                foreach (System.Windows.Forms.Control c in cbs)
                {
            ...
            foreach (DataColumn dc in _dt.Columns)
            {
                ...
                foreach (KeyValuePair<string, bool> kv in _transforms)
                {
                    if (kv.Value)
                    {
                        ...
                        {
                            abc = new List<double>();
                            skip = doTransform(out varlist, _dt, _dt.Columns.IndexOf(dc), _depVarColIndex, out adjr2, out r2, out abc, out pearsonR);
                            if (!skip)
                            {
                                vararray = varlist.ToArray();
                                //if (adjr2 < 0.1)
                                ...
                                //new pearson r stat via MC 12/2015
                                Pcoeff = pearsonR;
        ...
        /// <returns></returns>
        private bool doTransform(out List<double> vals, DataTable dt, int colndx, int depVarColIndex, out double adjr2, out double r2, out List<double> abc, out double pearsonR)
        {
