...
namespace VBCommon.Controls
{
    ...
    public partial class frmPCPlots : Form
    {
        ...
        public frmPCPlots(string mainVarName, List<string> varNames, string depVarName, DataTable dt, DataTable corrResults, out string message)
        {
            InitializeComponent();
            ...
            renderPlots(out message);
            //if (message != string.Empty) MessageBox.Show(message, MessageBoxIcon.Warning.ToString());
        }
        ...
        /// in the listbox
        /// </summary>
        private void renderPlots(out string message)
        {
            ...
            {
                GraphPane gp = addPlotXY(depvals, kv.Value, tags, kv.Key, out message);
                master.Add(gp);
                if (message != string.Empty) saveMsg = message;
            }
        ...
        private void showListInfo()
        {
            ...
            ListViewItem lvADPV;
            lvADPV = new ListViewItem("A-D Normality P-Value");
            lv1.Items.Add(lvADPV);
            ...
            foreach (KeyValuePair<string, double[]> kv in _plotData)
            {
                ...
                //get the ad stat/pval and add to list - added both ad and adPval for mc 11/20/2014 - mog
                double [] ad = getADstat(kv.Key.ToString());
                string fmtstring = formatNumberString(ad[0], 0);
                string s = string.Format(fmtstring, ad[0]);
                lvAD.SubItems.Add(s);
                //lvAD.SubItems.Add(ad.ToString("n4"));
                fmtstring = formatNumberString(ad[1], 0);
                s = string.Format(fmtstring, ad[1]);
                lvADPV.SubItems.Add(s);
        ...
        {
            //changed 11/21/2014 by mog via mc to return both AD statistic & Pval
            double [] retval = {0.0d, 0.0d};
            ...
            if (!adtest.ADStat.Equals(double.NaN)) retval[0] = adtest.ADStat; 
            if (!adtest.ADStatPval.Equals(double.NaN)) retval[1] = adtest.ADStatPval;
            return retval;
        }
        ...
        private GraphPane addPlotXY(double[] response, double[] iv, string[] tags, string title, out string retMessage)
        {
            retMessage = string.Empty;
            PointPairList ppl = new PointPairList();
            string tag = string.Empty;
            double corrTest = getCorrResults(title);

            GraphPane gp = new GraphPane();

            //if (_dtCopy.Rows.Count > 6 && (response.Covariance(iv) / response.StandardDeviation() / iv.StandardDeviation() < 1.0))
            if (_dtCopy.Rows.Count > 6 && (corrTest < 1.0))
            {
                ...
                //double pcoeff1 = getCorrResults(title);
                double pcoeff = corrTest;
            ...
            //else if (response.Covariance(iv) / response.StandardDeviation() / iv.StandardDeviation() >= 1.0)
            else if (corrTest >= 1.0)
            {
                retMessage = "This independent variable or one or more of its transforms is highly\n" +
                            "correlated with the response variable (Pearson's coefficient >= 1.0).\n" + 
                            "No scatter plot regression lines can be generated for these plots. Review\n" +
                            "your imported data; use of the variable or its correlated transform will\n" +
                            "likely lead to programmatic difficulties with model building.";
            }
        ...
        private void cbSelectPlot_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (_init) return;
            redoPlots(cbSelectPlot.SelectedItem.ToString(), out message);
            if (message != string.Empty)
                MessageBox.Show(message, MessageBoxIcon.Warning.ToString());
        }
        ...
        /// </summary>
        /// <param name="p"></param>
        private void redoPlots(string p, out string message)
        {
            message = string.Empty;
            MasterPane master = zgc1.MasterPane;
            master.PaneList.Clear();
