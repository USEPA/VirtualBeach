...
namespace VBCommon.Controls
{
    public partial class DatasheetControl : UserControl
    {
        ...
        public void seePlot(object o, EventArgs e)
        {
            ...
            frmPlot frmplot = new frmPlot(intResponseVarColIndex, intSelectedColIndex, filterDataTableRows(dt), out message);
            ...
            //added some warnings for boneheads that import/create rv-iv duplicate values in sheet - mog 11/21/2014
        ...
        public void UnTransform(object o, EventArgs e)
        {
            ...
            if (dlgr == DialogResult.Yes)
            {
                ...
                //Re-enable all of the original IV columns
                dtCopy = setHiddenIVstoUnhidden(dtCopy);
                ...
                if (colndx >= 1)
                {
                    ...
                    if (colname == strResponseVarColName)
                    {
                        ...
                        if (testValueAttribute(dt.Columns[intResponseVarColIndex], VBCommon.Globals.DEPENDENTVARIBLETRANSFORM))
                        {
                            ...
                            cmforResponseVar.MenuItems[3].Enabled = false; //we can't define a transform, since there's is an applied transform
                        }
                        else
                        {
                            cmforResponseVar.MenuItems[2].Enabled = false; //but cannot untransform a response variable
                            cmforResponseVar.MenuItems[3].Enabled = true; //we can define a transform, since there's no applied transform
                        }
        ...
        public DataTable setHiddenIVstoUnhidden(DataTable dt)
        {
            ...
            foreach (DataColumn dc in dtCopy.Columns)
            {
                ...
                if (hasAttr)
                {
                    hasAttr = dc.ExtendedProperties.ContainsKey(VBCommon.Globals.ENABLED);
                    if (hasAttr)
                    {
                        if (dc.ExtendedProperties[VBCommon.Globals.ENABLED].ToString() == "False")
                        {
                            dc.ExtendedProperties[VBCommon.Globals.ENABLED] = true;
                        }
                    }
                }
            }
        ...
        {
            Font font = dgv.Columns[e.ColumnIndex].InheritedStyle.Font;
            SizeF size = dgv.CreateGraphics().MeasureString(dgv.Columns[e.ColumnIndex].HeaderText, font);
            dgv.Columns[e.ColumnIndex].Width = Convert.ToInt32(size.Width);
        }
    }
}
