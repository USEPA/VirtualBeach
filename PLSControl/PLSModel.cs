...
namespace PLSControl
{
	public class PLSModel : RModelInterface
	{
        ...
        public Dictionary<string, object> PackState(string SerializedModel = null)
        {
            ...
            dictPackedState.Add("ncomp", ncomp);
            dictPackedState.Add("ncomp_max", ncomp_max);
        ...
        public void UnpackState(Dictionary<string, object> PackedState)
        {
            ...
            if (PackedState.ContainsKey("data"))
            {
                Type objType = typeof(Dictionary<string, double[]>);
                string jsonRep = PackedState["data"].ToString();

                object objData = JsonConvert.DeserializeObject(jsonRep, objType);
                this.data_dict = (Dictionary<string, double[]>)objData;
            }
            ...
            this.ncomp = Convert.ToInt32(PackedState["ncomp"]);
            this.ncomp_max = Convert.ToInt32(PackedState["ncomp_max"]);
		...
		public void CrossValidation(int cv_method=0)
		{
			...
			if (cv_method==1)
			{
				...
				double PRESS0 = Convert.ToDouble(validation["PRESS0"].AsNumeric()[0]);
