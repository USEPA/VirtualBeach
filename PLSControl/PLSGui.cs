...
namespace PLSControl
{
    ...
    public partial class PLSGui : RModelingGui
    {
        public PLSGui() : base()
        {   
            InitializeComponent();
		...
		protected override void PopulateResults(RModelInterface Model)
        {
            Cursor.Current = Cursors.WaitCursor;
            PLSModel model = (PLSModel)Model;
            ...
            List<string> lstNames = model.Extract("names");
            List<double> lstCoefficients = model.Extract("coef");

            //Run the Get_Influence method to get the relative influence (coefficient x standard dev.) of each variable
            Dictionary<string, double> dictInfluence = model.GetInfluence();
            List<string> lstKeys = dictInfluence.Keys.ToList();
            List<double> lstInfluence = dictInfluence.Values.ToList();
            ...
            lblDecisionThreshold.Text = String.Format("{0:F3}", VBCommon.Transforms.Apply.UntransformThreshold(model.Threshold, base.xfrmImported, base.dblImportedPowerTransformExponent));
